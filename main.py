...
#    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & ~filters.ChatType.GROUPS,suggest_add_to_feed), group=1)
    ...
    application.add_handler(CallbackQueryHandler(commands.settings_menu, pattern="^settings$"))
    application.add_handler(CallbackQueryHandler(commands.language_menu, pattern="^language_menu$"))
    application.add_handler(CallbackQueryHandler(commands.set_language, pattern=r"^set_language_(ru|en)$"))
