...
async def process_channel_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # ===== ВАРИАНТ 2: Обычный текст (@username, ссылки, через запятую) =====
    # Парсим несколько каналов (через запятую, перенос строки, или пробел)
    raw_inputs = []
    for delimiter in [',', '\n', ' ']:
        if delimiter in channel_input:
            raw_inputs = [c.strip() for c in channel_input.split(delimiter) if c.strip()]
    ...
    if not raw_inputs:
        await update.message.reply_text("❌ Ничего не распознал. Пришли @username, ссылку, или перешли пост из канала.")
        return GET_CHANNEL_INPUT
    ...
    if telethon_manager and telethon_manager.is_started:
        await telethon_manager.refresh_handlers()
    logger.info(f"Done. Added: {len(added)}, Failed: {len(failed)}")
    # Формируем ответ
    response_lines = []
    if added:
        response_lines.append(f"✅ Добавлено {len(added)}:")
